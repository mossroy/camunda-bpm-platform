...
public class ProcessSetRemovalTimeJobHandler extends AbstractBatchJobHandler<SetRemovalTimeBatchConfiguration> {
  ...
  public void executeHandler(SetRemovalTimeBatchConfiguration batchConfiguration,
                             ...
                             String tenantId) {
    if (batchConfiguration.isUpdateInChunks()) {
      ...
      Integer chunkSize = getUpdateChunkSize(commandContext);
      Map<Class<? extends DbEntity>, DbOperation> operations = addRemovalTimeToInstance(instanceId, batchConfiguration, chunkSize, entities, commandContext);
      MessageEntity currentJob = (MessageEntity) commandContext.getCurrentJob();
      registerTransactionHandler(batchConfiguration, operations, chunkSize, currentJob, commandContext);
  ...
  protected Map<Class<? extends DbEntity>, DbOperation> addRemovalTimeToInstance(String instanceId, SetRemovalTimeBatchConfiguration batchConfiguration, Integer batchSize, Set<String> entities, CommandContext commandContext) {
  ...
  protected void registerTransactionHandler(SetRemovalTimeBatchConfiguration batchConfiguration,
    ...
    TransactionListener transactionResulthandler = createTransactionHandler(batchConfiguration, operations, chunkSize, currentJob, newCommandExecutor);
    commandContext.getTransactionContext().addTransactionListener(TransactionState.COMMITTED, transactionResulthandler);
  }

  protected int getUpdateChunkSize(CommandContext commandContext) {
    return commandContext.getProcessEngineConfiguration().getRemovalTimeUpdateChunkSize();
  }

  protected TransactionListener createTransactionHandler(SetRemovalTimeBatchConfiguration batchConfiguration,
      Map<Class<? extends DbEntity>, DbOperation> operations,
      Integer chunkSize,
      MessageEntity currentJob,
      CommandExecutor newCommandExecutor) {
    return new ProcessSetRemovalTimeResultHandler(operations, chunkSize, currentJob.getId(), batchConfiguration, newCommandExecutor, this);
  }
